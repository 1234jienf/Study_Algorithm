...
# 민규가 지불해야 하는 금액의 최댓값을 구하는 프로그램을 작성하시오. 
...
def max_price(N, card_lst):
    # dp[i]는 카드 i개를 구매할 때의 최대 금액
    dp = [0] * (N+1)
    
    # 카드 1개부터 N개까지 최대 금액을 계산
    for i in range(1, N+1):
        for j in range(1, i+1):
            dp[i] = max(dp[i], dp[i - j] + card_lst[j - 1])
    
    return dp[N]

## 가질수 있는 카드 팩 조합
print(max_price(N, card_lst))
